// Full-stack host-runtime test: mounts the surface-probe fixture (both
// transports) through the real translator+runtime instantiation path, and
// exercises the round trip described in fixtures/surface-probe/src/lib.rs's
// module doc (op sequence + event summary format — the authority for the
// exact assertions below).
//
// Requires `just fixtures` to have built
// fixtures/build/surface-probe-{stream,call}.component.wasm first.
...
const FIXTURES = [
  { name: "stream", path: "../../fixtures/build/surface-probe-stream.component.wasm" },
  { name: "call", path: "../../fixtures/build/surface-probe-call.component.wasm" },
] as const;
    ...
    if (e instanceof Deno.errors.NotFound) {
      throw new Error(
        `fixture component not found at ${url}. Run \`just fixtures\` first ` +
          `(builds fixtures/build/surface-probe-{stream,call}.component.wasm).`,
      );
    }
    throw e;
  }
...
for (const fixture of FIXTURES) {
  Deno.test(`fullstack (${fixture.name} transport): mount, event round trip, ordering`, async () => {
    const root = makeRoot();
    const componentBytes = await loadComponentBytes(fixture.path);
    ...
    // 5) Stream lane only: the direct-read path decoded the whole delivered
    // view into complete frames — nothing staged.
    if (fixture.name === "stream") {
