//! Ported from DioxusLabs/dioxus @ v0.7.10,
//! examples/01-app-demos/todomvc.rs (MIT/Apache-2.0). Ported verbatim
//! except for the following minimal changes required to run as a
//! polyengine-dioxus lib-crate component instead of a native dioxus-desktop
//! binary:
//!
//!   1. Dropped `const STYLE: Asset = asset!("/examples/assets/todomvc.css");`
//!      and the `Stylesheet { href: STYLE }` node inside `app()`. The
//!      `asset!` macro needs the `dx` CLI's asset pipeline (link-time asset
//!      collection), which this repo's plain `cargo build` component
//!      pipeline (`just example`) does not run. The E2E harness page links
//!      `todomvc.css` directly instead (see harness/entry.ts) — same
//!      styling, no `dx`-specific build step.
